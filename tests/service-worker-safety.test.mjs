...
test('service worker cleans only old Life Hub cache versions',async()=>{
 const w=worker();await w.event('activate');assert.deepEqual(w.deleted,['lifehub-v34','lifehub-v35']);
});
test('service worker precaches encrypted finance profiles and their shared assets',async()=>{
 const w=worker();await w.event('install');for(const path of ['finance/index.html','partner/index.html','plan/index.html','finance/money-map.js','finance/money-map.css','finance/shared-budget.js','finance/ui-safety.js','partner-sync.js'])assert.ok(w.precache.includes('./'+path),path);
});

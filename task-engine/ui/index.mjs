...
function list(title,tasks,empty='Nothing here — lovely.',overflow=0) { return `<section><h2>${title} <span>${tasks.length+overflow}</span></h2>${tasks.length?tasks.map(taskCard).join(''):`<div class="empty">${empty}</div>`}${overflow?`<button class="choice" data-action="nav" data-view="tasks">${overflow} more · View all tasks</button>`:''}</section>`; }
function todayHtml() { const p=buildTodayPlan(store.getState().tasks); return `<div class="hero"><div><small>GENERATED FOR YOU</small><h1>Today Plan</h1><p>Deadlines first. Five at a time, with nothing hidden.</p></div><button class="primary" data-action="add">＋ Add task</button></div>${list('Must do',p.mustDo,undefined,p.overflow.mustDo)}${list('Should do',p.shouldDo,undefined,p.overflow.shouldDo)}${list('Could do',p.couldDo,undefined,p.overflow.couldDo)}`; }
